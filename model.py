...
class Decoder(nn.Module):
    def __init__(self, skip_num=2, out_planes=3):
        ...
        self.convs = nn.ModuleList([
            ...
            #conv3x3(32 * skip_num, 16)
            ])
        self.last_conv = nn.Conv2d(
            #16 * skip_num, out_planes, kernel_size=3, padding=1
...
if __name__ == '__main__':
    ...
    encoder = Encoder()
    decoder = Decoder(skip_num=2, out_planes=4)

    with torch.no_grad():
        x = torch.rand(2, 6, 512, 1024)
        en_list = encoder(x)
        de_list = decoder(en_list[::-1])
